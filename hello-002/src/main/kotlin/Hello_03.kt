...
fun main() {
     ...
     *  KT에서는 객체를 생성할때 new 키워드를 사용하지 않는다
     *  
     * KT Random은 객체를 생성할때 seed 값을 매개변수로 필요로 한다
     *  seed는 random 값을 생성할때 내부적으로 연산에 사용하는 값 
     */
    var rnd:Random = Random(System.currentTimeMillis())

    // 난수 생성
    // 0 ~ 99까지 범위에서 한개의  정해지지 않은 수(임의의 수)를 생성한다
    var num1 = rnd.nextInt(until=100)

    print("생성된 난수 : $num1")

    // 생성ㄷ왼 난수가 짝수인가 홀수인가 판별
    if(num1 % 2 == 0 ) {
    ...
    /**
     * Kotlin에서는 if 가 식(expression)
     *  expression : 어떤 결과를 return하는 연산식
     *  일반적인 3항 연산자처럼  if문을 사용하는 독특한 방법
     */
